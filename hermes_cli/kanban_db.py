...
def update_task_fields(
    ...
    *,
    ...
    ``assignee=_FIELD_UNSET`` leaves the assignee unchanged; ``assignee=None``
    unassigns. Refuses an assignee change while the task is running (same rule
    as :func:`assign_task`). Title and body may still be edited on a running card.
    """
    ...
    with write_txn(conn):
        ...
        if profile is not _FIELD_UNSET:
            _append_event(
                ...
                {"assignee": profile, "from": row["assignee"]},
            )
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    """``{"name", "on_disk", "counts"}`` for every on-disk profile, hub lane,
    or task assignee, so a fresh profile or hub lane appears in pickers
    before it has a task."""
    ...
    return [
        ...
        for name in sorted(on_disk | hub | set(counts))
    ]
