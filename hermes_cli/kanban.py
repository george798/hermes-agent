...
_DELEGATED_CHILD_DENIED_ACTIONS: frozenset[str] = frozenset({
    ...
    "claim", "comment", "attach", "attach-rm", "complete", "edit", "update", "block",
...
def _cmd_update(args: argparse.Namespace) -> int:
    if args.title is None and args.body is None and args.assignee is None:
        return _err("kanban update: pass --title, --body, and/or --assignee", 2)
    ...
    with kbc.connect_closing() as conn:
        ...
        if not ok:
            return _err(f"no such task: {args.task_id}")
        task = kb.get_task(conn, args.task_id)
    if _json_out(args, _task_to_dict(task) if task else {}):
        return 0
...
_HANDLERS = {
    ...
    "complete": _cmd_complete, "edit": _cmd_edit, "update": _cmd_update, "block": _cmd_block,
