...
def _has_spawnable(conn: sqlite3.Connection, status: str) -> bool:
    ...
    if not rows:
        return False
    nonspawnable = _kb._hub_nonspawnable_lanes()
    rows = [row for row in rows if row["assignee"] not in nonspawnable]
    if not rows:
        return False
...
def _dispatch_lane_task(
    ...
    *,
    ...
    profile_exists = _profile_exists_fn()
    # Hub lanes whose kind is not ``hermes`` (interactive IDEs, fleet CLIs)
    # must never auto-spawn, even when a Hermes profile of the same name exists.
    if assignee in _kb._hub_nonspawnable_lanes() or (
        profile_exists is not None and not profile_exists(assignee)
    ):
...
def _any_spawnable_review(
    ...
    *,
    ...
    for row in review_rows:
        assignee = row["assignee"]
        if not assignee or assignee in _kb._hub_nonspawnable_lanes():
            continue
