 ...
 *  whole roster. Recomputed every round so a member pulled in
 *  mid-conversation still joins the next round. */
export function resolveGroupResponders(log: GroupMessage[], members: GroupMember[], round = 0) {
...
export async function runGroupChatRounds(
  ...
  try {
    for (let round = 0; round < GROUP_CHAT_MAX_ROUNDS; round++) {
      ...
      const responders = rotateGroupSpeakers(resolveGroupResponders(roomLog, members, round), round).filter(
