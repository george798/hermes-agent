...
describe('routing', () => {
  ...
  it('later rounds only wake members mentioned after the user message', async () => {
    const { rounds } = await loadRoom()

    const opened: GroupMessage[] = [
      { at: 1, from: { kind: 'user', name: 'You' }, text: 'hello team' },
      { at: 2, from: { kind: 'bot', name: 'builder' }, text: 'on it' }
    ]

    expect(rounds.resolveGroupResponders(opened, MEMBERS, 0)).toHaveLength(3)
    expect(rounds.resolveGroupResponders(opened, MEMBERS, 1)).toEqual([])

    const pulled: GroupMessage[] = [
      ...opened,
      { at: 3, from: { kind: 'bot', name: 'builder' }, text: '@theops your turn' }
    ]

    expect(rounds.resolveGroupResponders(pulled, MEMBERS, 1).map(member => member.name)).toEqual(['ops'])
  })
